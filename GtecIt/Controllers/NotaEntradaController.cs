      ...
      /*  public ActionResult CreatedentistaEdicao(long orcamento_id)
        ...
        */
        ...
        public ActionResult Create(NotaEntradaCreateViewModel model, FormCollection form)
        {
            ...
            itens.Id_stqentra= id_orcamento;
            itens.cd_almox = 0;
            itens.cd_produto = 100;
        ...
        public ActionResult Delete(int codigo)
        {
           ...
           model.status = "2";
                //itens
           _uoW.NotaEntradas.Salvar(Mapper.Map<NotaEntrada>(model));
                
           var model_itens =
           Mapper.Map<List<NotaEntradaItemEditViewModel>>(
               _uoW.NotaEntradaItems.ObterTodos()
                ...
                foreach (var itens in model_itens)
                {
                    ...
                    _uoW.NotaEntradaItems.Atualizar(Mapper.Map<NotaEntradaItem>(itens));
                }
                //titulos
                var model_titulos =
           Mapper.Map<List<TituloapagarEditViewModel>>(
               _uoW.Titulosapagar.ObterTodos()
                   .Where(x => x.Id_stqnoten== codigo)
                   .ToList()
                   .OrderBy(x => x.Id_stqnoten)).ToList();
                ...
                {
                    itens.Status = "2";
                    _uoW.Titulosapagar.Atualizar(Mapper.Map<Tituloapagar>(itens));
                }
            _uoW.Complete();
