...
namespace GtecIt.Controllers
{
    public class OrcamentoController : Controller
    {
        ...
        public ActionResult Index(OrcamentoIndexViewModel model, string tipoacao)
        {
            ...
            try
            {
                model.Grid =
                    Mapper.Map<List<OrcamentoGridViewModel>>(
                       _uoW.Orcamentos.ObterTodos()
                            .Where(x => x.id_Stqcporcamento.Equals(model.id_Stqcporcamento) && (x.status.Equals("0")))
                            .ToList()
                            .OrderBy(x => x.id_Stqcporcamento));
            }
            catch (Exception e)
        ...
        public ActionResult Edit(int codigo)
        {
            ...
            var model = Mapper.Map<OrcamentoEditViewModel>(_uoW.Orcamentos.ObterTodos().FirstOrDefault(x => x.id_Stqcporcamento == codigo && x.status.Equals("0")));
        ...
        {
            var model = Mapper.Map<OrcamentoEditViewModel>(_uoW.Orcamentos.ObterPorId(codigo));
            ModelState.Clear();
            if (!TryValidateModel(model))
            {
                var validationErrors = string.Join(",",
           ...
           var model_itens =
           Mapper.Map<List<OrcamentoItemEditViewModel>>(
               _uoW.OrcamentoItens.ObterTodos()
                ...
                {
                    itens.status = "2";
                    _uoW.OrcamentoItens.Atualizar(Mapper.Map<OrcamentoItem>(itens));
                }
                //titulos
                var model_titulos =
           Mapper.Map<List<TituloEditViewModel>>(
               _uoW.Titulos.ObterTodos()
                ...
                {
                    itens.status = "2";
                    _uoW.Titulos.Atualizar(Mapper.Map<Titulo>(itens));
                }
            _uoW.Complete();
